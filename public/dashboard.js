...
async function loadLogs() {
  try {
    ...
    if (bidData.bids && bidData.bids.length > 0) {
      bidLogsDiv.innerHTML = bidData.bids
        .map(
          (log) => `
    ...
    if (errorData.errors && errorData.errors.length > 0) {
      errorLogsDiv.innerHTML = errorData.errors
        .map(
          (log) => `
...
async function checkAuthAndLoadQuotes() {
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        "[CHECK AUTH] Dashboard visibility set to:",
  ...
  for (const quote of quotes) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/bidding-status/${quote.enquiry_number}`
      );
      const data = await response.json();
      ...
      if (data.active) {
        console.log(
          `[DASHBOARD] Restoring active bidding for ${quote.enquiry_number}`
        );
      ...
      console.error(
        `Error checking bidding status for ${quote.enquiry_number}:`,
        error
      );
    }
  }
...
async function startSmartBidding(enquiryNumber, closingTimestamp) {
  ...
  if (
    ...
    for (let index = 0; index < quote.unit_details.charges.length; index++) {
      const lowInput = document.querySelector(
        `input.cargo-input[data-enquiry="${enquiryNumber}"][data-cargo-index="${index}"][data-type="low"]`
      );
      const mediumInput = document.querySelector(
        `input.cargo-input[data-enquiry="${enquiryNumber}"][data-cargo-index="${index}"][data-type="medium"]`
      );
      const highInput = document.querySelector(
        `input.cargo-input[data-enquiry="${enquiryNumber}"][data-cargo-index="${index}"][data-type="high"]`
      );

      if (!lowInput?.value || !mediumInput?.value || !highInput?.value) {
        alert(
          `Please fill all bid values for ${quote.unit_details.charges[index].type}`
        );
        return;
      }
    ...
    bids = {
      low: document.querySelector(
        `input[data-enquiry="${enquiryNumber}"][data-type="low"]`
      ).value,
      medium: document.querySelector(
        `input[data-enquiry="${enquiryNumber}"][data-type="medium"]`
      ).value,
      high: document.querySelector(
        `input[data-enquiry="${enquiryNumber}"][data-type="high"]`
      ).value,
    };
...
document
  ...
  .addEventListener("click", async () => {
    ...
    if (tokenData.hasGlobalToken) {
      ...
      console.log(
        "[AUTH] Existing token failed, proceeding with re-authentication..."
      );
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        document.getElementById("authNote").textContent =
          "Chrome is running in the background. Please check your email for the OTP.";
...
document.getElementById("submitOtpBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (otpResponse.ok) {
      ...
      document.getElementById("authNote").textContent =
        "Authentication successful! Loading quotes...";
      setTimeout(() => {
        checkAuthAndLoadQuotes();
      }, 1000);
    } else {
...
function resetAuthForm() {
  ...
  document.getElementById("authenticateBtn").textContent =
    "Authenticate with Chrome";
  document.getElementById("otpInput").value = "";
  document.getElementById("authNote").textContent =
    "Chrome will run in the background. An OTP will be sent to your email.";
}
...
function updateBiddingUI(enquiryNumber, status, isActive) {
  ...
  const inputs = document.querySelectorAll(
    `input[data-enquiry="${enquiryNumber}"]`
  );
  const statusDiv = document.getElementById(`status-${enquiryNumber}`);
...
function matchesLocationFilter(quote) {
  ...
  return (
    origin.includes(plantName.toLowerCase()) ||
...
async function calculateBidPrices(marketRateInput, cargoIndex) {
  ...
  try {
    ...
    // Get the correct bid percentages - use cargo index to determine bid type
    // Assuming you want to use bid_1, bid_2, etc. based on cargoIndex
    const bidKey = `bid_${cargoIndex + 1}`;
    const bidData = pricePercents[bidKey] || { high: 9, medium: 7, low: 5 };

    // Calculate bid prices
    const highPrice = Math.round(marketRate * (1 + bidData.high / 100));
    const mediumPrice = Math.round(marketRate * (1 + bidData.medium / 100));
    const lowPrice = Math.round(marketRate * (1 + bidData.low / 100));

    console.log(
      `[CALC] Cargo ${cargoIndex}: Market ₹${marketRate} -> High ₹${highPrice}, Medium ₹${mediumPrice}, Low ₹${lowPrice} (using ${bidKey})`
    );
...
async function saveMarketRate(enquiryNumber, cargoIndex, marketRate) {
  try {
    ...
    if (!response.ok) {
      console.error(
        "[SAVE MARKET RATE] Failed to save market rate:",
        response.statusText
      );
    } else {
      console.log(
