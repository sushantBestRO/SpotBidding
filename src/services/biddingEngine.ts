...
class BiddingEngine {
  ...
  public async startBidding(
    ...
    // Fetch pricing settings for initial bid (bid_1)
    const config = await this.getConfig();
    const pricePercents = config.pricePercents || {};
    const initialBidData = pricePercents["bid_1"] || {
      high: 0,
      medium: 0,
      low: 0,
    };

      logBid(enquiryKey, `Using bid_1 percentages for initial bidding: High ${initialBidData.high}%, Medium ${initialBidData.medium}%, Low ${initialBidData.low}%`);
